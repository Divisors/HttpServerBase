...
public final class Decoder {
	...
	private static final IOException MAX_DYNAMIC_TABLE_SIZE_CHANGE_REQUIRED = new IOException(
			"max dynamic table size change required");
		...
		while (in.available() > 0) {
			...
			case READ_LITERAL_HEADER_NAME_LENGTH_PREFIX:
				...
				if (index == 0x7f) {
					...
					// Disallow empty names -- they cannot be represented in
					// HTTP/1.x
					if (nameLength == 0) {
						throw DECOMPRESSION_EXCEPTION;
					}
	...
	/**
	 * End the current header block. Returns if the header field has been
	 * truncated. This must be called after the header block has been completely
	 * decoded.
	 */
	...
	/**
	 * Set the maximum table size. If this is below the maximum size of the
	 * dynamic table used by the encoder, the beginning of the next header block
	 * MUST signal this change.
	 */
	...
	/**
	 * Return the maximum table size. This is the maximum size allowed by both
